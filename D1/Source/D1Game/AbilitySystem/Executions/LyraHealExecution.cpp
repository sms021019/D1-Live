...
struct FHealStatics
{
	...
	FHealStatics()
	{
		//BaseHealDef = FGameplayEffectAttributeCaptureDefinition(UD1CombatSet::GetBaseHealAttribute(), EGameplayEffectAttributeCaptureSource::Source, true);
	}
};
...
void ULyraHealExecution::Execute_Implementation(const FGameplayEffectCustomExecutionParameters& ExecutionParams, FGameplayEffectCustomExecutionOutput& OutExecutionOutput) const
{
	...
	if (HealingDone > 0.0f)
	{
		...
		//OutExecutionOutput.AddOutputModifier(FGameplayModifierEvaluatedData(UD1VitalSet::GetHealingAttribute(), EGameplayModOp::Additive, HealingDone));
	}
